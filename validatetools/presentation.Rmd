...
subtitle: "Manage, check and resolve contradictory rule sets" 
...
- `validate` allows to store and manage the rules explicitly, external to the data processing.
- Rules can be stored in a R file, yaml text-file, or a data.frame (e.g. from a data base).
- Rules can be partitioned in subdomains and be included as needed.
## Growing pains
...
### At Statistics Netherlands / CBS:

- Datasets with > 100 columns/variables are common.
- Some systems have 100s of rules.
- Often multiple persons work on rule set.
- Often specialized rules for sub-domains.
...
## validatetools methods

- `is_infeasible` => `make_feasible`
- `detect_redundancy` => `remove_redundancy`
- `detect_fixed_variables` => `simplify_fixed_variables`
- `substitute_values`, `simplify_rules`, `simplify_conditional`
...
## Assignment: Can you simplify these rules with `validatetools`?
...
a) Apply `simplify_rules(rules, job = "yes")`
...
b) Apply `simplify_rules(rules, age = 10)`
